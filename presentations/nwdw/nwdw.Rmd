...
source("C:/Users/smshi/Dropbox/Research/BD-RMG-Women/codes/00_factory_data.R", local = knitr::knit_global())
```
...
ggplot(bd_full) +
  geom_sf() +
  geom_sf(data = filter(factory_geo, date_est<=1990), color = "black", size = 0.5) +
  geom_sf(data = filter(factory_geo, date_est>1990 & date_est<=2000), color = "red") +
  theme_map() + labs(title="Factories betwen 90-00")
...
Export exposure $X_{c,t}$ depends on levels of export originating from a location

X_{r,t}     &= Export \space Shock_{r,t} \text{ and} \\
Export \space Shock_{r,t} &= \frac{\alpha_{r,t}^{knit} * \Delta Knit \space Export_{t-T} + (1 - \alpha_{r,t}^{knit}) * \Delta Woven \space Export_{t-T}}{Population_{r,t}} \\
