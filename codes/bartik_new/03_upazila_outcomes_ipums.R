...
fac_up_sample01$ipum1991[fac_up_sample01$ipum1991 == 20019072] <- 20019067
...
upazila_data <- left_join(ever_married, fertility_df, by = c("ipum1991", "year")) %>%
  left_join(general_data, by = c("ipum1991", "year")) %>%
  left_join(human_cap, by = c("ipum1991", "year")) %>%
  left_join(lfp_1520, by = c("ipum1991", "year")) %>%
  left_join(lfp_1529, by = c("ipum1991", "year")) %>%
  left_join(lfp_1564, by = c("ipum1991", "year")) %>%
  left_join(factory_upazilas %>%
              select(area91, ipum1991), by = c("ipum1991"))

upazila_data %>%
  mutate(year = remove_labels(year)) -> upazila_data
  
save(upazila_data, file = "C:/Users/smshi/Dropbox/Research/BD-RMG-Women/data/upazila_data_ipums.RData")